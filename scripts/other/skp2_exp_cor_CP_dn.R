...
#make out dir
outdir <- "nonsurvival/skp2cor_tkoenrichment/cor_dn"
...
pways_to_show = c('GOBP_OUTFLOW_TRACT_MORPHOGENESIS', 'GOBP_AORTA_DEVELOPMENT',
                  'GOBP_RESPONSE_TO_RETINOIC_ACID', 'GOBP_CELL_CELL_SIGNALING_BY_WNT',
                  'GOBP_REGULATION_OF_BMP_SIGNALING_PATHWAY',
                  'GOBP_RESPONSE_TO_BMP'
)

cnetplot(obj, showCategory = pways_to_show, foldChange = namedfc,
         shadowtext='category',
         cex_gene = 0.5, cex_label_gene = 0.5, cex_label_category = 0.7)
...
pdf(paste0(outdir, '/specificpathways_genenetworkplots.pdf'), width = 8, height = 8)
...
pways_to_show = c('GOBP_REGULATION_OF_BMP_SIGNALING_PATHWAY',
                  'GOBP_RESPONSE_TO_BMP',
...
### try to combine mapk and BMP...
upnamedfc <- namedfc

## we'll use the clust8_down script to get that object and res into memory
objdn <- readRDS('E95/integrated/clusterprofiler/cluster8_down/clusterprofilerobj.rds')
downnamedfc <- namedfc

namedfc <- c(upnamedfc, downnamedfc)
...
c1 <- cnetplot(obj, showCategory = pways_to_show, foldChange = upnamedfc,
               shadowtext='category',
               cex_gene = 0.5, cex_label_gene = 0.8, cex_label_category = 0.6)

pways_to_show = c('GOBP_NEGATIVE_REGULATION_OF_MAPK_CASCADE',
                  'GOBP_NEGATIVE_REGULATION_OF_MAP_KINASE_ACTIVITY'
)

c2 <- cnetplot(objdn, showCategory = pways_to_show, foldChange = downnamedfc,
               shadowtext='category',
               cex_gene = 0.5, cex_label_gene = 0.8, cex_label_category = 0.6)

pdf(paste0(outdir, '/specificpathways_BMP-MAPK.pdf'), width = 6, height = 4.5)
c1
c2
dev.off()
