...
# ie, for each gene, get the rows, and take the ROW WITH HIGHEST MEAN?
total = length(dups)
...
for(geneidx in 1:length(dups)) {
  ...
  sg <- as.matrix(dupgem[rownames(dupgem) %in% sdg$probeset,])
  
  cm <- data.frame(sg[which.max(rowMeans(sg)),])
...
### scores we will calculate:
# TKO overexp
# TKO underexp
# MCP COUNTER SCORES
# TKO overexp without myeloid
# TKO overexp with only myeloid
# SKP2 (just gene not score)
# PTPRC (just gene not score)
# AIF1 (just gene not score)
...
#also just bind expressin of some key genes
genes <- c('SKP2', 'PTPRC', 'AIF1')



mm <- cbind(mm,  t(gem[genes,]) )
...
#clean up env
rm(c8, editedmodules, genes_nodups, myeloid, res, down, dup_hgnc, dup_mgi, up, genes, up_nomyeloid, up_onlymyeloid)
...
outdir <- "survivalanalysis/kuijjer/TKOvsDKO"
dir.create(outdir)


suboutdir <- paste0(outdir, '/', 'probemax_qnorm_limmabc_vitalstatus')
...
suboutdir <- gsub('_vitalstatus', '_metfree', suboutdir)
