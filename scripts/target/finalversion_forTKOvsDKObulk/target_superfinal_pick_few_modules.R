...
#### we'll pick a few modules:


# TKO overexp
# TKO overexp onlymyeloid
# monocyte lineage
# T cells

# also make dich plots with line at 5 years...

# for target, only vital status....


#set outdir
outdir <- 'survivalanalysis/FINAL_TKO_VS_DKO/superfinal_target'
dir.create(outdir)
...
###select few scores from mm

scores_to_test <- c('TKO_overexpressed', 'TKO_overexpressed_onlymyeloid', 'Monocytic_lineage', 'CD8_T_cells')
mm <- mm[,colnames(mm) %in% scores_to_test]
...
for(scoreidx in 1:length(sigpways) ){
  ...
  pdf(paste0(resultdir, '/gplots.pdf'), height = 4.5, width = 5)
  print(gdich)
  print( gdich + Seurat::NoLegend() )
  print(gterc)
  dev.off()
